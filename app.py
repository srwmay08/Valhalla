...
from config import ICO_SUBDIVISIONS
from config import STARTING_UNITS
from config import NEUTRAL_GARRISON
from config import TERRAIN_COLORS
from config import SCALES
from config import SURFACE_OCEANS
from config import MIN_SURFACE_DEEP_SEA_PERCENT
from config import MAX_SURFACE_DEEP_SEA_PERCENT
from config import SPAWN_CHANCE_WASTE
from config import SPAWN_CHANCE_FARM
from config import MOUNTAIN_RANGE_MIN_LENGTH
from config import MOUNTAIN_RANGE_MAX_LENGTH
from config import TIER_2_THRESHOLD
from config import TIER_3_THRESHOLD
from config import PASSIVE_GROWTH
from config import FLOW_RATE
...
# Ensure AI (Orc) is Green/distinct
...
def generate_game_world():
    ...
    if land_indices:
        ...
        if components:
            ...
            for small_comp in components[1:]:
                for idx in small_comp:
                    face_terrain[idx] = "Deep Sea"
                    ocean_tiles.add(idx)
    ...
    if forest_seeds:
        ...
        for _ in range(num_forests):
            ...
            for idx in cluster:
                if face_terrain[idx] not in ["Deep Sea", "Sea"]:
                    face_terrain[idx] = "Forest"
    ...
    for i in final_land_indices:
        if random.random() < SPAWN_CHANCE_WASTE:
            face_terrain[i] = "Waste"
...
def background_thread():
    while True:
        ...
        with thread_lock:
            ...
            # Run AI Logic (Imported from ai_engine.py)
            ...
            for fid, fort in game_state["fortresses"].items():
                ...
                if fort['paths']:
                    for target_id in fort['paths']:
                        ...
                        if target['owner'] == fort['owner']:
                            ...
                            if target['race'] == "Neutral":
                                defender_race = {"base_def": 1.0}
                ...
                old_tier = fort['tier']
                new_tier = 1
                if current_units >= TIER_3_THRESHOLD:
                    new_tier = 3
                elif current_units >= TIER_2_THRESHOLD:
                    new_tier = 2
...
def create_app():
    ...
    def assign_home_base(user):
        ...
        for i, face in available_faces:
            ...
            if game_state["fortresses"][v1]['owner'] or game_state["fortresses"][v2]['owner']:
                continue
