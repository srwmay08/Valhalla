...
export class GameRenderer {
    ...
    highlightPathHover(pathId, color = 0xffffff) {
        ...
        if (line) { line.material.opacity = 1.0; line.material.emissive.setHex(color); }
    }

    highlightConnectedPaths(sourceId, color = 0xffffff) {
        const prefix = `path_${sourceId}_`;
        Object.keys(this.pathLines).forEach(key => {
            if (key.startsWith(prefix)) { this.pathLines[key].material.opacity = 1.0; this.pathLines[key].material.emissive.setHex(color); }
        });
    }

    highlightFaceHover(faceIdx) { this.currentHoveredFace = faceIdx; }
    highlightFaceSelection(faceIdx) { this.currentSelectedFace = faceIdx; }
    clearHoverHighlight() {
        Object.values(this.fortressMeshes).forEach(g => g.children[1].material.emissive.setHex(0));
        Object.values(this.pathLines).forEach(l => { l.material.opacity = 0.6; l.material.emissive.setHex(0); });
        this.currentHoveredFace = null;
    }
    clearSelectionHighlights() { this.currentSelectedFace = null; }
    ...
    updateFaceColors(faceColors) {
        ...
        faceColors.forEach((colorHex, i) => {
            ...
            const base = i * 9;
            for(let j=0; j<3; j++) { colors[base + j*3] = color.r; colors[base + j*3 + 1] = color.g; colors[base + j*3 + 2] = color.b; }
        });
    ...
    updateFortresses(fortressData, currentUsername) {
        Object.values(fortressData).forEach(fort => {
            ...
            if (label) {
                ...
                dots.forEach((dot, i) => { dot.style.background = (i < fort.tier) ? '#ffffff' : '#444444'; });
            }
        });
    }
    ...
    updatePathVisuals(fort, teamColor) {
        ...
        for (const key in this.pathLines) {
            if (key.startsWith(keyPrefix)) { this.scene.remove(this.pathLines[key]); delete this.pathLines[key]; }
        }
        ...
        fort.paths.forEach(targetId => {
            ...
            const material = new THREE.MeshLambertMaterial({ color: teamColor, transparent: true, opacity: 0.6, emissive: new THREE.Color(0x000000) });
    ...
    updatePackets(edgeData) {
        ...
        Object.values(edgeData).forEach(edge => {
            ...
            const start = new THREE.Vector3(...this.vertices[edge.u]), end = new THREE.Vector3(...this.vertices[edge.v]);
